...
/* exposed $.mobile methods	 */
	...
	$.mobile.loadPage = function( url, options ) {
		...
		if ( !( $.mobile.allowCrossDomainPages || path.isSameDomain( documentUrl, absUrl ) ) ) {
			...
			// Load the new page.
			$.ajax({
				url: fileUrl, // + (new Date()).getTime() to prevent caching
				type: settings.type,
				data: settings.data,
				dataType: "json",
				async: false,
				...
				success: function( json ) {
					...
					var pageTemplate = json.template; // this will hold the new template that needs to be loaded.

					//console.log('pageTemplate = ' + pageTemplate + '.html');
					var pagehtml = '';
					var headerTemplate = '';
					var footerTemplate = '';
             console.log('page template ' + pageTemplate);
					...
					$.ajax({
						...
						success: function(data){
                           ...
                           error: function(){
                           console.log("footer load error");
                           }
					});
					$.ajax({
						url: 'views/' + pageTemplate + '.html?'+ (new Date()).getTime(),
						dataType: 'text',
						async: false,
						success: function(data){
                           console.log('in success');
							pageTemplateHtml = headerTemplate + data + footerTemplate;
							console.log(pageTemplateHtml);
							pagehtml = Mustache.to_html(pageTemplateHtml, pageData);
							//pagehtml = $("<div />").html(pagehtml);
							console.log('*HERE-----------------');
...
//					return;
					...
					//var pageContent = "<div data-" + $.mobile.ns + "role='page' id='" + pageId + "'";
					//if(cachePage)
					//	pageContent +=	 'data-dom-cache="true"';
					//pageContent += ">" + pagehtml + "</div>"
					
				//	alert(pageContent);
				//	page = $( pageContent );
             //embedding campaign and company id for image upload
            var ids = '';
             if(pageId == 'campaign-details'){
              ids = " data-campaignId='" + pageData.data.campaign.id + "' + data-companyId='" + pageData.data.campaign._company + "' ";
             }
						page = $( "<div " + ids + "data-" + $.mobile.ns + "role='page' id=" + pageId + ">" + pagehtml + "</div>" );
						//page = page.html();
					//}
